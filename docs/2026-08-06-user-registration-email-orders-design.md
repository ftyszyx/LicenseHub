...
`referral_code` 为可选字段，只用于新用户注册时建立永久邀请关系。只有分销总开关和“永久邀请绑定”开关同时开启时才会生效；已经存在的账号不能通过登录、购买或管理员接口补绑邀请人。
...
4. 再次检查用户名和邮箱唯一性。
5. 分销开启且推广码有效时，锁定邀请用户并写入永久邀请关系；无效或缺失时保持未绑定，注册后不能补绑。
6. 创建用户并写入 `email_verified_at`。
7. 分配普通用户角色并生成推广码。
8. 认领该邮箱的有效游客订单，但不改写历史订单原有的推广归因和佣金。
9. 消费 challenge 和注册凭证。
10. 提交事务后签发 JWT。

永久邀请关系只允许在该注册事务中首次写入。migration 不为历史用户回填邀请人，管理员新增用户也不会建立邀请关系，现有用户更新接口不提供绑定、换绑或解绑字段。

管理员“系统设置”中的“永久邀请绑定”开关默认关闭。关闭只阻止新的注册绑定，不解除已有关系；分销总开关关闭时，已有关系保留但新订单不产生佣金。
