  ...
  <div class="space-y-4">
    ...
    <el-card shadow="never">
      <el-table :data="rows" stripe size="large" style="width: 100%" @selection-change="onSelChange">
        ...
        <template v-if="query.code_type !== RegCodeType.Count">
          <el-table-column prop="valid_days" :label="$t('reg_codes.valid_days')" width="120"> </el-table-column>
        ...
        <template v-if="query.code_type !== RegCodeType.Time">
          <el-table-column prop="total_count" :label="$t('reg_codes.total_count')" width="120"> </el-table-column>
        </template>
        ...
        <el-table-column :label="$t('common.actions')" width="180" fixed="right" align="right">
          <template #default="{ row }">
            <div class="flex justify-end gap-2">
              <el-button v-if="row.status === RegCodeStatus.Unused" size="small" type="primary" plain @click="markUsed(row)">{{
                $t("reg_codes.mark_used")
              }}</el-button>
              <el-button v-if="row.status === RegCodeStatus.Unused" size="small" type="danger" plain @click="del(row.id)">{{
                $t("common.delete")
              }}</el-button>
            </div>
          </template>
    ...
    <el-dialog v-model="batch.visible" :title="$t('reg_codes.batch_create')" width="520px">
      <el-form label-width="140px">
        ...
        <el-form-item v-if="batchAppCodeType === RegCodeType.Time" :label="$t('reg_codes.valid_days')"
          ><el-input-number v-model.number="batch.valid_days" :min="1"
        /></el-form-item>
...
const query = reactive<ListRegCodesParams>({
  code: "",
  app_id: undefined,
  code_type: undefined,
});
...
function onSelChange(arr: RegCodeModel[]) {
...
function exportCsv() {
  ...
  const lines = rows.value.map((r) =>
    [r.id, r.code, r.app_id, r.app_name || "", r.code_type, r.valid_days, r.total_count ?? "", r.status, r.created_at].join(","),
  );
  ...
  a.href = url;
  a.download = "reg_codes.csv";
  a.click();
  URL.revokeObjectURL(url);
}

const batch = reactive<Omit<BatchCreateRegCodesReq, "code_type"> & { visible: boolean }>({
  visible: false,
  app_id: 0,
  quantity: 10,
  valid_days: 7,
...
const selectedBatchApp = computed(() => appOptions.value.find((item) => item.id === batch.app_id));
const batchAppCodeType = computed(() => selectedBatchApp.value?.code_type ?? RegCodeType.Time);
...
async function submitBatch() {
  if (!selectedBatchApp.value) {
    ElMessage.warning(t("common.please_check_form"));
    return;
  }
  await batchCreateRegCodes({
    ...batch,
    code_type: batchAppCodeType.value,
  });
  batch.visible = false;
