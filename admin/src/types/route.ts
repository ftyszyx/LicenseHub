export enum RouteName {
    Home = 'home',
    Products = 'products',
    AppProducts = 'app_products',
    OrderQuery = 'order_query',
    PayResult = 'pay_result',
    Login = 'login',
    Register = 'register',
    Admin = 'admin',
    User = 'user',
    UserHome = 'user_home',
    UserOrders = 'user_orders',
    UserDistribution = 'user_distribution',
    AdminDashboard = 'dashboard',
    AdminProducts = 'admin_products',
    AdminOrders = 'orders',
    AdminPaymentSettings = 'payment_settings',
    AdminStorageChannels = 'storage_channels',
    AdminSystemSettings = 'system_settings',
    AdminSystemLogs = 'system_logs',
    AdminUsers = 'users',
    AdminRoles = 'roles',
    AdminApps = 'apps',
    AdminResources = 'resources',
    AdminRegCodes = 'reg_codes',
    AdminRegTest = 'reg_test',
    AdminUseRecords = 'use_records',
    AdminDevices = 'devices',
    AdminPermissions = 'permissions',
    AdminDistribution = 'distribution_commissions',
}


export enum RoutePath {
    Home = '/',
    Products = '/' + RouteName.Products,
    OrderQuery = '/orders/query',
    PayResult = '/pay/result',
    Login = '/' + RouteName.Login,
    Register = '/' + RouteName.Register,
    Admin = '/' + RouteName.Admin,
    User = '/' + RouteName.User,
    UserHome = '/' + RouteName.User + '/home',
    UserOrders = '/' + RouteName.User + '/orders',
    UserDistribution = '/' + RouteName.User + '/distribution',
    AdminDashboard = '/' + RouteName.Admin + '/' + RouteName.AdminDashboard,
    AdminProducts = '/' + RouteName.Admin + '/products',
    AdminOrders = '/' + RouteName.Admin + '/' + RouteName.AdminOrders,
    AdminPaymentSettings = '/' + RouteName.Admin + '/' + RouteName.AdminPaymentSettings,
    AdminStorageChannels = '/' + RouteName.Admin + '/' + RouteName.AdminStorageChannels,
    AdminSystemSettings = '/' + RouteName.Admin + '/' + RouteName.AdminSystemSettings,
    AdminSystemLogs = '/' + RouteName.Admin + '/' + RouteName.AdminSystemLogs,
    AdminUsers = '/' + RouteName.Admin + '/' + RouteName.AdminUsers,
    AdminRoles = '/' + RouteName.Admin + '/' + RouteName.AdminRoles,
    AdminApps = '/' + RouteName.Admin + '/' + RouteName.AdminApps,
    AdminResources = '/' + RouteName.Admin + '/' + RouteName.AdminResources,
    AdminRegCodes = '/' + RouteName.Admin + '/' + RouteName.AdminRegCodes,
    AdminRegTest = '/' + RouteName.Admin + '/' + RouteName.AdminRegTest,
    AdminUseRecords = '/' + RouteName.Admin + '/' + RouteName.AdminUseRecords,
    AdminDevices = '/' + RouteName.Admin + '/' + RouteName.AdminDevices,
    AdminPermissions = '/' + RouteName.Admin + '/' + RouteName.AdminPermissions,
    AdminDistribution = '/' + RouteName.Admin + '/' + RouteName.AdminDistribution,
}
